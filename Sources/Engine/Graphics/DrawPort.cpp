...
uint32_t CDrawPort::GetID(void)
{
  uint32_t ulCRC;
  CRC_Start(   ulCRC);
  CRC_AddLONG( ulCRC, (uint32_t)dp_Raster);
  CRC_AddLONG( ulCRC, (uint32_t)dp_MinI);
  CRC_AddLONG( ulCRC, (uint32_t)dp_MinJ);
  CRC_AddLONG( ulCRC, (uint32_t)dp_MaxI);
  CRC_AddLONG( ulCRC, (uint32_t)dp_MaxJ);
  CRC_Finish(  ulCRC);
  return ulCRC;
}
...
void CDrawPort::DrawPoint( PIX pixI, PIX pixJ, COLOR col, PIX pixRadius/*=1*/) const
{
  ...
  if( eAPI==GAT_OGL) {
    ...
    const uint32_t d3dColor = rgba2argb(col);
...
// draw one line
void CDrawPort::DrawLine( PIX pixI0, PIX pixJ0, PIX pixI1, PIX pixJ1, COLOR col, uint32_t typ/*=_FULL*/) const
{
  // check API
  const GfxAPIType eAPI = _pGfx->gl_eCurrentAPI;
#ifdef SE1_D3D
  ...
  if( typ==_FULL_) {
    ...
    extern void gfxSetPattern( uint32_t ulPattern); 
    gfxSetPattern(typ);
...
// draw border
void CDrawPort::DrawBorder( PIX pixI, PIX pixJ, PIX pixWidth, PIX pixHeight, COLOR col, uint32_t typ/*=_FULL_*/) const
{
  // check API
  const GfxAPIType eAPI = _pGfx->gl_eCurrentAPI;
#ifdef SE1_D3D
                      ...
                      COLOR colUL, COLOR colUR, COLOR colDL, COLOR colDR) const
{
  ...
  if( eAPI==GAT_OGL) {
    ...
    // thru Direct3D
    HRESULT hr;
    const uint32_t d3dColUL = rgba2argb(colUL);  const uint32_t d3dColUR = rgba2argb(colUR);
    const uint32_t d3dColDL = rgba2argb(colDL);  const uint32_t d3dColDR = rgba2argb(colDR);
    CTVERTEX avtxTris[6] = {
...
void CDrawPort::GrabScreen( class CImageInfo &iiGrabbedImage, INDEX iGrabZBuffer/*=0*/) const
{
  ...
  const int32_t slBytes  = pixPicSize * iiGrabbedImage.ii_BitsPerPixel/8;
  iiGrabbedImage.ii_Picture = (uint8_t*)AllocMemory( slBytes);
  ...
  if( eAPI==GAT_OGL)
  {
    ...
    if( bGrabDepth) {
      ...
      OGL_CHECKERROR;
      // convert
      uint8_t *pubZBuffer = (uint8_t*)pfZBuffer;
      for( INDEX i=0; i<pixPicSize; i++) pubZBuffer[i] = 255-NormFloatToByte(pfZBuffer[i]);
      // add as alpha channel
      AddAlphaChannel( iiGrabbedImage.ii_Picture, (uint32_t*)iiGrabbedImage.ii_Picture,
  ...
  else if( eAPI==GAT_D3D)
  {
    ...
    int32_t slColSize;    
    uint8_t *pubSrc = (uint8_t*)rectLocked.pBits;
    uint8_t *pubDst = iiGrabbedImage.ii_Picture;
    // loop thru rows
    for( INDEX j=0; j<dp_Height; j++) {
      // loop thru pixles in row
      for( INDEX i=0; i<dp_Width; i++) {
        uint8_t ubR,ubG,ubB;
        extern COLOR UnpackColor_D3D( uint8_t *pd3dColor, D3DFORMAT d3dFormat, int32_t &slColorSize);
...
uint32_t CDrawPort::GetTextWidth( const CTString &strText) const
{
  // prepare scaling factors
  PIX   pixCellWidth    = dp_FontData->fd_pixCharWidth;
  int32_t fixTextScalingX = FloatToInt(dp_fTextScaling*dp_fTextAspect*65536.0f);
  ...
  { // get current letter
    ...
    // special char encountered and allowed?
      ...
      case 'c':  i += FindZero((uint8_t*)&strText[i],6);  continue;
      case 'a':  i += FindZero((uint8_t*)&strText[i],2);  continue;
      case 'f':  i += 1;  continue;
...
void CDrawPort::PutText( const CTString &strText, PIX pixX0, PIX pixY0, const COLOR colBlend) const
{
  ...
  int32_t fixTextScalingX = FloatToInt(fTextScalingX  *65536.0f);
  int32_t fixTextScalingY = FloatToInt(dp_fTextScaling*65536.0f);
  PIX pixCellWidth  = dp_FontData->fd_pixCharWidth;
  ...
  uint32_t ulAlphaDefault = (colBlend&CT_AMASK)>>CT_ASHIFT;;  // for flasher
  ...
  BOOL bItalic  = FALSE;
  INDEX iFlash  = 0;
  uint32_t ulAlpha = ulAlphaDefault;
  ...
  for( INDEX iChar=0; iChar<ctMaxChars; iChar++)
  {
    ...
    // special char encountered and allowed?
      ...
      case 'c':
        ...
        iRet = FindZero( (uint8_t*)&strText[iChar+1], 6);
        iChar+=iRet;
      ...
      case 'a':
        ...
        iRet = FindZero( (uint8_t*)&strText[iChar+1], 2);
        iChar+=iRet;
                            ...
                            const COLOR colUL, const COLOR colUR, const COLOR colDL, const COLOR colDR) const
{
  ...
  (int32_t&)fCorrectionU = (127-FastLog2(pixWidth))  <<23; // fCorrectionU = 1.0f / ptd->GetPixWidth() 
  (int32_t&)fCorrectionV = (127-FastLog2(pixHeight)) <<23; // fCorrectionV = 1.0f / ptd->GetPixHeight() 
...
void CDrawPort::BlendScreen(void)
{
  ...
  uint32_t fix1oA = 65536 / dp_ulBlendingA;
  uint32_t ulRA = (dp_ulBlendingRA*fix1oA)>>16;
  uint32_t ulGA = (dp_ulBlendingGA*fix1oA)>>16;
  uint32_t ulBA = (dp_ulBlendingBA*fix1oA)>>16;
  uint32_t ulA  = ClampUp( dp_ulBlendingA, 255UL);
