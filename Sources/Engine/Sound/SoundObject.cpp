                             ...
                             FLOAT fMaxVolume, FLOAT fPitch)
{
  ...
  if (_bPredictionActive && so_penEntity!=NULL) {
    if (so_penEntity->IsPredictionHead()) {
      ...
      if (pen!=so_penEntity) {
        pso = (CSoundObject *)( ((uint8_t*)pen) + (int(this)-int(so_penEntity)) );
      }
    }
  }
...
CSoundObject *CSoundObject::GetPredictionTail(uint32_t ulTypeID, uint32_t ulEventID)
{
  ...
  if (so_penEntity!=NULL) {
    ...
    if (pen->IsPredictor()) {
      ...
      if (pen!=so_penEntity) {
        //CPrintF(" ROUTED\n");
        return (CSoundObject *)( ((uint8_t*)pen) + slOffset );
      }
    }
  }
...
void CSoundObject::Play(CSoundData *pCsdLink, SLONG slFlags)
{
  ...
  // get prediction tail
  CSoundObject *psoTail = GetPredictionTail(EVENT_SOUNDPLAY, (uint32_t)pCsdLink);
  // if the event is predicted
  if (psoTail==NULL) {
    // do nothing;
    return;
...
void CSoundObject::Stop(void)
{
  ...
  // get prediction tail
  psoTail = GetPredictionTail(EVENT_SOUNDSTOP, (uint32_t)so_pCsdLink);
  // if the event is predicted
  if (psoTail==NULL) {
    // do nothing;
    return;
